...
- After editing markerlite_gui.py: `python check_gui.py`. After editing
  markerlite.py: run the fixture regression (below) and diff outputs.
...
## Test fixtures (not in repo yet — first task is to add them under tests/)

Generated with fpdf2 / pdflatex; each exists to reproduce a specific fixed bug:
`hard.pdf` (two-column, running head, hyphenation across column, table,
footnotes), `paper.pdf` (real pdflatex, display equations, booktabs table),
`repro.pdf` (3 pages, page-top headings, raster figure + caption, vector chart,
Symbol+Times equation), `repro_tight.pdf` (same, 11mm top margin — header
merges into the heading block), `footnote_repro.pdf` (note wrapped across two
blocks, superscript refs, an exponent), `scanned.pdf` (raster of hard.pdf, OCR
path), `manuscript.pdf` (double-spaced, margin line numbers, first-line
indents, running head drawn last). Regression = convert all, diff against the
previous outputs; they should be byte-identical unless the change intends
otherwise. Generators live in the Cowork session history; recreate them from
the descriptions above (fpdf2 for all but paper.pdf, which is LaTeX).
...
- Add fixtures + a `tests/regress.py` to the repo; add `python check_gui.py`
  as a CI step before the build.
